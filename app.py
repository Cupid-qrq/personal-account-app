...
from src.auth import authenticate_user, can_upload, get_user_role
...
<style>
    * {
    ...
    /* 登录卡片 */
    .login-card {
        background: linear-gradient(135deg, rgba(104, 211, 255, 0.1), rgba(76, 255, 200, 0.1));
...
if not st.session_state.logged_in:
    ...
    with col2:
        ...
        username = st.text_input("账户名", placeholder="输入用户名", key="login_username")
...
with st.sidebar:
    st.markdown("### 👤 用户信息")
    st.write(f"**账户:** {st.session_state.username}")
    st.write(f"**角色:** {'管理员 (可上传)' if st.session_state.user_role == 'admin' else '访客 (仅查看)'}")
    ...
    if can_upload(st.session_state.user_role):
        ...
        with tab1:
            ...
            if uploaded is not None and st.button("✅ 导入数据", key="upload_btn"):
                ...
                st.success(
                    f"✓ 成功导入 {result['imported_rows']} 条\n"
                    f"📅 月份: {', '.join(result['months_saved']) or '无'}\n"
                    f"📊 总数: {result['master_rows']}"
                )
                st.rerun()
        
        with tab2:
            if st.button("🔄 扫描并导入", key="batch_btn"):
    ...
    with col1:
        if st.button("⭐ GitHub", key="github_link", use_container_width=True):
            st.markdown(
                "[⭐ Star on GitHub](https://github.com/Cupid-qrq/personal-account-app)",
                unsafe_allow_html=True,
            )
    with col2:
        st.write("")
...
if master_df.empty:
    st.markdown("⚠️ 暂无数据，请上传账单文件")
    st.stop()

# 页面标题
st.markdown("# 💳 个人财务分析系统")
st.markdown("*AI 驱动的消费洞察*")

months = sorted(master_df["月份"].dropna().unique().tolist())
selected_month = st.selectbox("📅 选择月份", options=months, index=len(months) - 1)
...
for col, (label, value) in zip(cols, metrics):
    with col:
        ...
        <div class="metric-card">
            ...
            <div class="metric-value">¥{value if '¥' not in value else value.replace('¥', '')}</div>
        </div>
        """, unsafe_allow_html=True)
...
with tab_overview:
    ...
    with pie_col:
        ...
        if not cat_df.empty:
            ...
            st.markdown("**查看详情：**")
            cat_cols = st.columns(min(len(cat_df), 3))
    ...
    if not daily_df.empty:
        ...
        fig_line.update_traces(
            ...
            line=dict(color="#68d3ff", width=2),
            marker=dict(size=6),
        )
...
if st.session_state.selected_category:
    st.markdown("---")
    st.markdown(f"### 🔍 {st.session_state.selected_category} 详情")
...
with tab_insight:
    ...
    with insight_col2:
        st.metric("总支出", f"¥{habit.get('total_expense', 0):.0f}", delta=None)
        st.metric("日均消费", f"¥{habit.get('avg_daily_expense', 0):.0f}", delta=None)
    
    with insight_col3:
        st.metric("平均单笔", f"¥{habit.get('avg_per_transaction', 0):.0f}", delta=None)
        st.metric("消费天数", f"{habit.get('expense_days', 0)}", delta=None)
...
with tab_detail:
    ...
    if st.button("📂 展开/收起", key="detail_toggle"):
    ...
    top_col, budget_col = st.columns(2)
    
    with top_col:
        st.markdown("**🔥 Top 10 高额支出**")
