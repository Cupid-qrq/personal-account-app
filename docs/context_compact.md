# Context Compact - 账本管理系统 v0.7

更新时间: 2026-04-05

## 项目目标
- 完成账单 CSV 的导入、清洗、去重、按月归档、主表汇总。
- 提供可视化分析与消费洞察，支持管理员上传、访客查看。
- 在移动端和桌面端保持可用的分析体验。

## 当前版本状态
- 版本: v0.7（SQLite 分层重构中）
- 主入口: app.py（仅做导入入口）
- UI 层: src/ui_app.py
- 分析模块: src/analytics.py
- 数据流水线: src/data_pipeline.py
- 数据契约: src/data_contract.py
- SQLite 存储: src/sqlite_store.py
- 认证模块: src/auth.py

## v0.7 核心更新
1. 主数据层切换到 SQLite，CSV 继续保留为归档与外部镜像。
2. 数据契约集中到 `src/data_contract.py`，统一原始 CSV 与派生字段。
3. 入口文件瘦身为启动器，UI 逻辑迁入 `src/ui_app.py`。
4. 现有分析函数继续复用 `src/analytics.py`，避免重写分析口径。

## v0.5+ 核心更新
1. 视觉层升级：动态背景、浮动光斑、3D 质感指标卡、Hero 信息区。
2. 月度分析升级为四维中心：趋势总览、结构透视、消费节律、智能洞察。
3. 新增数据函数：monthly_category_share、monthly_rhythm_heatmap、monthly_insight_digest。
4. 保留并延续 v0.5 的稳定性修复与数据归档成果。

## v0.5 核心更新
1. 修复金额重复显示: 概览指标卡不再二次拼接金额字符串。
2. 修复导入排序崩溃: 统一合并阶段时间列类型，避免 Timestamp/str 混排报错。
3. 修复预警边界异常: 无支出月份返回稳定 0 指标，避免 NaN。
4. 新增月度分析: 月度收入/支出/结余趋势、储蓄率、环比变化。
5. 完成 2026 年 1-2 月归档导入。

## 数据现状
- 主表: data/processed/ledger_master.csv
- 主表记录数: 99
- 覆盖月份: 2026-01, 2026-02, 2026-03
- 月份分布:
  - 2026-01: 35
  - 2026-02: 25
  - 2026-03: 39
- 归档文件:
  - data/archive/2026-01.csv
  - data/archive/2026-02.csv
  - data/archive/2026-03.csv

## 功能结构
- 登录与权限:
  - 管理员: 上传/导入 + 查看分析
  - 访客: 仅查看分析
- 主界面四大板块:
  - 支出分析
  - 消费洞察
  - 风险预警
  - 详细账目
- 新增月度洞察中心:
  - 趋势总览（多月趋势 + 环比）
  - 结构透视（占比堆叠 + 3D 结构）
  - 消费节律（周序 x 周几热力）
  - 智能洞察（排名 + 集中度 + 波动结论）

## 已知限制
- 密码仍为硬编码，后续建议迁移到环境变量或数据库。
- SQLite 已接入，但写入仍以单进程顺序更新为前提，未做并发锁优化。

## 封版发布整理
1. README 已改为对外版本，避免内部协作信息外泄。
2. 新增 LICENSE（MIT），补齐发布必要文件。
3. .gitignore 已升级，忽略本地环境/缓存/敏感配置文件。
4. v0.5 作为阶段完成版，下阶段目标为 v1.0。

## 建议下一步
1. 补充 SQLite 到 CSV 的回放/迁移脚本。
2. 继续拆分 UI 页面，减少 `src/ui_app.py` 的单文件体积。
3. 增加季度/年度同比与导出能力。

## 迭代发布工作流（强制）
1. 完成功能或修复后，必须先执行基础校验（语法/关键数据检查）。
2. 校验通过后，必须执行 commit 并 push 到远程主分支。
3. 若线上连接 Streamlit Cloud，push 后检查自动部署状态；失败则手动 Redeploy。
4. README 仅保留对外发布信息，不暴露内部调试细节与演示凭证。

## iteration-workflow Skill
- 已创建项目级 skill：.github/skills/iteration-workflow/SKILL.md
- 用途：迭代时先分析代码，再实现、校验、更新文档/记忆，最后 commit + push
- 触发方式：在 Copilot 对话中直接要求“使用 iteration-workflow skill 处理当前项目迭代”
